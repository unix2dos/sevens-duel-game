...
function inkColor(card: Card) {
  return suitInk(card) === "warm" ? "#c64835" : "#1c2733";
}
...
function pipMarkup(card: Card) {
  ...
  return points
    .map((point) => {
      ...
      return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle" font-family="'Times New Roman', serif" font-size="40" fill="${color}" transform="${rotation}">${symbol}</text>`;
    })
    .join("");
}
...
function aceMarkup(card: Card) {
  return `
    <text x="${ART_WIDTH / 2}" y="${ART_HEIGHT / 2 - 12}" text-anchor="middle" dominant-baseline="middle"
      font-family="'Times New Roman', serif" font-size="110" fill="${inkColor(card)}">${suitSymbol(card.suit)}</text>
    <text x="${ART_WIDTH / 2}" y="${ART_HEIGHT / 2 + 54}" text-anchor="middle" dominant-baseline="middle"
      font-family="'Cormorant Garamond', serif" font-size="24" letter-spacing="3" fill="#8f7b63">ACE</text>
  `;
}
...
function faceMarkup(card: Card) {
  ...
  return `
    <g transform="translate(${ART_WIDTH / 2} ${ART_HEIGHT / 2})">
      ...
      <path d="M0 -84 L16 -52 L0 -58 L-16 -52 Z" fill="#b99a65" opacity="0.9"/>
      <path d="M0 84 L16 52 L0 58 L-16 52 Z" fill="#b99a65" opacity="0.9"/>
      <text x="0" y="-28" text-anchor="middle" font-family="'Cormorant Garamond', serif" font-size="78" font-weight="700" fill="${color}">${rankText(card.rank)}</text>
      <text x="0" y="24" text-anchor="middle" font-family="'Times New Roman', serif" font-size="72" fill="${color}">${emblem}</text>
      <text x="0" y="68" text-anchor="middle" font-family="'IBM Plex Sans', sans-serif" font-size="16" letter-spacing="4" fill="#8f7b63">${label}</text>
      <text x="0" y="-118" text-anchor="middle" font-family="'Times New Roman', serif" font-size="28" fill="${color}">${emblem}</text>
      <text x="0" y="136" text-anchor="middle" font-family="'Times New Roman', serif" font-size="28" fill="${color}" transform="rotate(180 0 136)">${emblem}</text>
    </g>
  `;
}
  ...
  return `
    <g ${transform}>
      <text x="0" y="0" text-anchor="middle" dominant-baseline="middle"
        font-family="'Sora', 'Helvetica Neue', sans-serif" font-size="34" font-weight="700" fill="${color}">${rankText(card.rank)}</text>
      <text x="0" y="28" text-anchor="middle" dominant-baseline="middle"
        font-family="'Times New Roman', serif" font-size="28" fill="${color}">${suitSymbol(card.suit)}</text>
    </g>
  `;
}
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${ART_WIDTH}" height="${ART_HEIGHT}" viewBox="0 0 ${ART_WIDTH} ${ART_HEIGHT}">
      <defs>
        <linearGradient id="paper" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <stop offset="100%" stop-color="#f4eadb"/>
        </linearGradient>
        ...
        <linearGradient id="medallion" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <stop offset="100%" stop-color="#f0e4d1"/>
        </linearGradient>
        <pattern id="paperNoise" width="24" height="24" patternUnits="userSpaceOnUse">
          <circle cx="5" cy="6" r="1" fill="#eadcc7" opacity="0.28"/>
          <circle cx="18" cy="11" r="1" fill="#eadcc7" opacity="0.18"/>
          <circle cx="12" cy="19" r="1" fill="#eadcc7" opacity="0.22"/>
        </pattern>
      </defs>
      <rect x="6" y="10" width="${ART_WIDTH - 12}" height="${ART_HEIGHT - 12}" rx="28" fill="#080a0e" fill-opacity="0.18"/>
      <rect x="0" y="0" width="${ART_WIDTH}" height="${ART_HEIGHT}" rx="26" fill="url(#paper)" stroke="#d8c3a5" stroke-width="2.4"/>
      <rect x="8" y="8" width="${ART_WIDTH - 16}" height="${ART_HEIGHT - 16}" rx="20" fill="none" stroke="#eadcc8" stroke-width="1.4"/>
...
export function buildCardBackSvg() {
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${ART_WIDTH}" height="${ART_HEIGHT}" viewBox="0 0 ${ART_WIDTH} ${ART_HEIGHT}">
      <defs>
        <linearGradient id="backBase" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stop-color="#1f3046"/>
          <stop offset="100%" stop-color="#101a28"/>
        </linearGradient>
        <linearGradient id="gold" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="#f1dbab"/>
          <stop offset="100%" stop-color="#b89458"/>
        </linearGradient>
        <pattern id="mesh" width="18" height="18" patternUnits="userSpaceOnUse">
          <path d="M0 9 H18 M9 0 V18" stroke="#36506d" stroke-width="0.8" opacity="0.35"/>
        </pattern>
      </defs>
      <rect x="6" y="10" width="${ART_WIDTH - 12}" height="${ART_HEIGHT - 12}" rx="28" fill="#080a0e" fill-opacity="0.2"/>
      <rect width="${ART_WIDTH}" height="${ART_HEIGHT}" rx="26" fill="url(#backBase)" stroke="#b89458" stroke-width="2.2"/>
      <rect x="12" y="12" width="${ART_WIDTH - 24}" height="${ART_HEIGHT - 24}" rx="20" fill="#1a2636" stroke="#36506d" stroke-width="1.6"/>
      <rect x="20" y="20" width="${ART_WIDTH - 40}" height="${ART_HEIGHT - 40}" rx="16" fill="url(#mesh)" stroke="#4c6888" stroke-width="1.2"/>
      <g transform="translate(${ART_WIDTH / 2} ${ART_HEIGHT / 2})">
        <circle r="74" fill="none" stroke="url(#gold)" stroke-width="4"/>
        <circle r="54" fill="none" stroke="url(#gold)" stroke-width="1.5" opacity="0.9"/>
        <path d="M0 -104 L18 -70 L0 -80 L-18 -70 Z" fill="url(#gold)"/>
        <path d="M0 104 L18 70 L0 80 L-18 70 Z" fill="url(#gold)"/>
        <text x="0" y="-6" text-anchor="middle" font-family="'Cormorant Garamond', serif" font-size="68" font-weight="700" fill="url(#gold)">VII</text>
        <text x="0" y="42" text-anchor="middle" font-family="'IBM Plex Sans', sans-serif" font-size="24" letter-spacing="6" fill="#d6c097">SEVENS</text>
      </g>
    </svg>
  `.trim();
}
...
export function preloadCardBackTexture() {
  ...
  if (!backTextureLoad) {
    backTextureLoad = loadSvgTexture(buildCardBackSvg()).then((texture) => {
      backTexture = texture;
      return texture;
    });
  }
